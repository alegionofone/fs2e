import { adoptItemToActor, unadoptItemFromActor } from "../../global/adoption/item-adoption.mjs";
...
const HISTORY_SLOT_LABELS = ["Upbringing", "Apprenticeship", "Early Career", "Tour of Duty", "Tour of Duty"];
...
export class FS2ECharacterSheet extends ActorSheet {
  ...
  async getData(options = {}) {
    ...
    const speciesItem = (this.actor.items ?? []).find((item) => item?.type === "species") ?? null;
    const planetItem = (this.actor.items ?? []).find((item) => item?.type === "planet") ?? null;
    const factionItem = (this.actor.items ?? []).find((item) => item?.type === "faction") ?? null;

    data.system = this.actor.system;
    data.view = data.view ?? {};
    data.view.species = speciesItem
      ? {
        id: speciesItem.id,
        name: speciesItem.name,
        uuid: speciesItem.uuid
      }
      : null;
    data.view.planet = planetItem
      ? {
        id: planetItem.id,
        name: planetItem.name,
        uuid: planetItem.uuid
      }
      : null;
    data.view.faction = factionItem
      ? {
        id: factionItem.id,
        name: factionItem.name,
        uuid: factionItem.uuid
      }
      : null;
    const historySlots = this._readHistorySlots();
    data.view.histories = getHistorySlotDefinitions().map((slot, idx) => ({
      idx,
      label: slot.label,
  ...
  async _clearSpecies() {
    ...
    for (const speciesItem of speciesItems) {
      await unadoptItemFromActor(this.actor, speciesItem);
    }
  ...
  async _onDropSpecies(event) {
    ...
    // Persist species-driven base/max directly on actor for immediate and durable adoption.
    await adoptItemToActor(this.actor, created ?? itemDoc);
  }
  ...
  async _deleteHistorySlotLinkedItems(slotIndex) {
    const slots = this._readHistorySlots();
